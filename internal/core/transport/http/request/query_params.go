...
// GetDateQueryParam extracts an time.Time query parameter
// from the HTTP request by its key taking the layout into account.
// It returns nil if the parameter is missing
// or an error if the value is not a valid time.Time variable.
func GetDateQueryParam(r *http.Request, key, layout string) (*time.Time, error) {
	param := r.URL.Query().Get(key)
	if param == "" {
		return nil, nil
	}
